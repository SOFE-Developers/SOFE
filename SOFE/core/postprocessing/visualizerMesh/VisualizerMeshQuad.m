classdef VisualizerMeshQuad < VisualizerMesh
  methods % constructor
    function obj = VisualizerMeshQuad(mesh)
  ...
  methods % display
    function show(obj)
      ...
      if size(top.nodes, 2) == 2
        h = trisurf(elem(:,[1 2 4 3]), top.nodes(:,1), top.nodes(:,2), zeros(size(top.nodes,1),1));
        view(0,90);
      else
        h = trimesh(elem(:,[1 2 4 3]), top.nodes(:,1), top.nodes(:,2), top.nodes(:,3));
      end
