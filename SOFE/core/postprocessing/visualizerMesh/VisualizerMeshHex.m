classdef VisualizerMeshHex < VisualizerMesh
  methods % constructor
    function obj = VisualizerMeshHex(mesh)
  ...
  methods % display
    function show(obj, varargin) % [loc]
      ...
      h = trimesh(fc(Is,[1 2 4 3]), top.nodes(:,1), top.nodes(:,2), top.nodes(:,3));
      set(h,'facecolor',[0.5 0.7 0.2],'edgecolor','k');
      hold on
      h = trimesh(fc(Ib,[1 2 4 3]), top.nodes(:,1), top.nodes(:,2), top.nodes(:,3));
      hold off
    ...
    function showCells(obj, I, flag)
      ...
      nodes = top.getEntity(0);
      h = trimesh(face(II{3},[1 2 4 3]),nodes(:,1),nodes(:,2),nodes(:,3));
      set(h,'facecolor','none','edgecolor','k');
      if flag
        for i = 1:numel(flag)
